...
class DataLinkNode : public rclcpp::Node {
 public:
  DataLinkNode() : Node("datalink") {
    ...
    ConnectionResult connection_result =
        mavsdk_.add_any_connection(this->get_parameter("connection_path").as_string());
 ...
 private:
  ...
  void discover_topics() {
    ...
    for (const auto& topic : topic_names_and_types) {
      ...
      if t (topic.name.find(prefix) != 0)
        continue;  // skip topics that don't start with the prefix

      if (subscribed_topics_.find(topic_name) != subscribed_topics_.end())
        continue;  // skip already subscribed topics
      ...
      for (const auto& type : topic_types) {
        if (subscribe_to_topic(topic_name, topic_types)) {
          ...
          RCLCPP_INFO(this->get_logger(), "Subscribed to topic: %s of type: %s", topic_name.c_str(),
                      topic_types.c_str());
          break;
        }
      }
 ...
 private:
  void link_setup(const std::shared_ptr<System>& system) {
    ...
    engine_starter_test_client_ =
        create_client<interfaces::action::RunStarterTest>("/h20pro/run_starter_test");
  }
